...
if (-not (Test-Path $config.SolutionFile)) {
    throw "Solution file not found: $($config.SolutionFile)"
}

Write-Host ""
Write-Host "Building Godot C# solution..." -ForegroundColor Cyan
Write-Host $config.SolutionFile

dotnet build $config.SolutionFile
