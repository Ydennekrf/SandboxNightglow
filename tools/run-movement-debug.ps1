...
if ([string]::IsNullOrWhiteSpace($config.MovementDebugScene)) {
    throw "MovementDebugScene is not configured."
}

& (Join-Path $PSScriptRoot "run-test-scene.ps1") -Scene $config.MovementDebugScene
